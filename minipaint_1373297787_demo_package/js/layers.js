function LAYER_CLASS(){
	...
	this.layer_add = function(name, data, type){
		...
		layer_active = LAYERS.length-1;
		document.getElementById(LAYERS[layer_active].name).getContext("2d").globalAlpha = 1;
		this.layer_renew();
		}
	...
	this.move_layer = function(direction){
		...
		if(LAYERS[layer_active].primary == 1) return false;
		layer_active = parseInt(layer_active);
		
		var layer_from = LAYERS[layer_active];
		var content = document.getElementById(LAYERS[layer_active].name);
		var parent = content.parentNode;
		
		
		if(direction == 'up'){
			if(LAYERS[layer_active-1] == undefined) return false;
			if(LAYERS[layer_active-1].primary == 1) return false;
			var layer_to = LAYERS[layer_active-1];
			parent.insertBefore(content, parent.firstChild);
			}
		else{
			if(LAYERS[layer_active+1] == undefined) return false;
			var layer_to = LAYERS[layer_active+1];
			var content = document.getElementById(LAYERS[layer_active+1].name);
			parent.insertBefore(content, parent.firstChild);
			}
	...
	this.layer_visibility = function(i, id){
	...
	this.select_layer = function(i){
		if(layer_active != i)
			layer_active = i;	//select
		else
			layer_active = 0;	//remove select
		this.layer_renew();
		}
	...
	this.update_info_block = function(){
		...
		html += '<span style="font-weight:bold;min-width:45px;display:block;float:left;">Mouse:</span> '+mouse_pos[0]+", "+mouse_pos[1]+"<br />";
		if(select_square != false){
			html += '<span style="font-weight:bold;min-width:45px;display:block;float:left;">XY:</span> '+select_square.x+", "+select_square.y+"<br />";
			html += '<span style="font-weight:bold;min-width:45px;display:block;float:left;">Area:</span> '+select_square.w+", "+select_square.h+"<br />";
			}
	...
	this.set_alpha = function(){
		POP.add({name: "param1",	title: "Alpha:",	value: LAYERS[layer_active].opacity,	range: [0, 1], });
		POP.show('Opacity', function(user_response){
			var param1 = parseFloat(user_response.param1);
			LAYERS[layer_active].opacity = param1;
